...
fn write_rig_and_pairs_with_options(
    ...
    let replace_generated_default = existing_rig_config.as_ref().is_none_or(|config| {
        config == &unknown_default || config == &deprecated_colocated_default
    });
    if replace_generated_default {
        ...
        let marker_matches = fs::read(&nominal_prior_path)
            .ok()
            .and_then(|bytes| serde_json::from_slice::<Value>(&bytes).ok())
            .and_then(|marker| marker.get("rigConfig").cloned())
            .as_ref()
            == existing_rig_config.as_ref();
        if !marker_matches {
            remove_align_artifact(&nominal_prior_path)?;
        }
    }
...
fn nominal_rig_prior_matches(root: &Path, configs: &[RigBootstrapConfig]) -> bool {
    ...
    marker.get("schemaVersion").and_then(Value::as_u64) == Some(1)
        && marker.get("mappingLocked").and_then(Value::as_bool) == Some(true)
        && marker
            .get("rigConfig")
...
fn run_align(
    ...
    // The nominal Insta360 prior uses a zero baseline because independent
    // monocular reconstructions do not provide metric scale. Refining that
    // unobservable sensor offset collapses this capture to a two-frame model;
    // keep all preconfigured sensor poses fixed during mapping.
    let disable_sensor_refinement = rig_preconfigured;
    ...
    if rig_preconfigured {
        ...
        emit_log(
            ...
            if nominal_rig_prior {
                "已在初始建模前套用 Insta360 背對背名義 rig；此先驗僅用於避免無基線的跨鏡初始化"
            } else {
                "已在初始建模前套用固定雙鏡頭相對外參"
            },
        );
