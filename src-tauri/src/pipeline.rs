...
fn write_rig_and_pairs_with_options(
    ...
    let existing_rig_config = rig_config
        .is_file()
        .then(|| fs::read(&rig_config).map_err(|error| error.to_string()))
        .transpose()?
        .and_then(|bytes| serde_json::from_slice::<Value>(&bytes).ok());
    let capture = fs::read(root.join("metadata/capture.json"))
        .ok()
        .and_then(|bytes| serde_json::from_slice::<Value>(&bytes).ok());
    let source_groups = capture
        .as_ref()
        .and_then(|value| value.get("sourceGroups"))
        .and_then(Value::as_array);
    let nominal_hint = source_groups.and_then(|groups| {
        let hints = groups
            .iter()
            .map(|group| {
                group
                    .get("adapter")
                    .and_then(Value::as_str)
                    .and_then(camera_adapter::rig_bootstrap_pose_hint)
            })
            .collect::<Option<Vec<_>>>()?;
        let first = *hints.first()?;
        hints.iter().all(|hint| *hint == first).then_some(first)
    });
    // Align results produced by the old unknown-rig path persisted the
    // database-derived extrinsics without marking the file as generated. If
    // that run identifies itself as bootstrap_mapper and the adapter now has
    // a deterministic nominal pose, migrate it on the next rerun. A manually
    // supplied complete rig never takes the bootstrap_mapper path, so this
    // does not overwrite user calibration.
    let legacy_generated_bootstrap = nominal_hint.is_some()
        && fs::read(root.join("metadata/align.checkpoint.json"))
            .ok()
            .and_then(|bytes| serde_json::from_slice::<Value>(&bytes).ok())
            .and_then(|checkpoint| {
                checkpoint
    ...
    if replace_generated_default {
        ...
        // supply a marked nominal bootstrap pose when their physical layout is
        // known but metric factory calibration is unavailable. Preserve every
        // other user-supplied config verbatim.
        let selected_config = nominal_hint.map_or_else(
        ...
        if let Some(hint) = nominal_hint {
            write_json_atomic(
                &nominal_prior_path,
                &json!({
                    "schemaVersion": 1,
                    "provenance": hint.provenance,
                    "rigConfig": selected_config,
                    "mappingLocked": !hint.refine_sensor_from_rig,
                    "refineSensorFromRig": hint.refine_sensor_from_rig
                }),
            )?;
        } else {
        ...
        let marker_matches = fs::read(&nominal_prior_path)
            .ok()
            .and_then(|bytes| serde_json::from_slice::<Value>(&bytes).ok())
            .is_some_and(|marker| {
                matches!(
    ...
    // A preconfigured back-to-back rig does not need feature matches between
    // its two mostly disjoint fisheyes to keep the sensors connected. Learned
    ...
    for (index, name) in names.iter().enumerate() {
        if !suppress_unverified_cross_lens_pairs {
...
fn nominal_rig_prior_matches(root: &Path, configs: &[RigBootstrapConfig]) -> bool {
    let Ok(marker) = fs::read(root.join("metadata/nominal_rig_prior.json")) else {
        return false;
    };
    let Ok(marker) = serde_json::from_slice::<Value>(&marker) else {
        return false;
    };
    marker.get("schemaVersion").and_then(Value::as_u64) == Some(1)
        ...
        && marker
            .get("rigConfig")
            .and_then(|value| serde_json::from_value::<Vec<RigBootstrapConfig>>(value.clone()).ok())
            .as_ref()
            .is_some_and(|marked| marked == configs)
}
...
fn nominal_rig_prior_refines_sensor(root: &Path, configs: &[RigBootstrapConfig]) -> bool {
    if !nominal_rig_prior_matches(root, configs) {
        return false;
    }
    fs::read(root.join("metadata/nominal_rig_prior.json"))
        .ok()
        .and_then(|bytes| serde_json::from_slice::<Value>(&bytes).ok())
...
fn run_align(
    ...
    if rig_preconfigured {
        ...
        emit_log(
            ...
            id,
            "info",
            if nominal_rig_prior {
                "已在初始建模前套用相機 adapter 的背對背名義 rig；此先驗僅用於避免無基線的跨鏡初始化"
            } else {
                "已在初始建模前套用固定雙鏡頭相對外參"
            },
        );
    ...
    if rig_mapping_plan == RigMappingPlan::BootstrapThenConfigure {
        ...
        emit_log(
            ...
            "info",
            if nominal_rig_prior {
                "相機組已有 adapter 提供的背對背名義 pose，略過無法觀測基線的獨立鏡頭 bootstrap"
            } else {
...
mod tests {
    ...
    #[test]
    fn insta360_capture_uses_a_marked_nominal_back_to_back_bootstrap_pose() {
        let temp = tempfile::tempdir().unwrap();
        for lens in ["lens0", "lens1"] {
            fs::create_dir_all(temp.path().join("images").join(lens)).unwrap();
            for name in ["frame0001.png", "frame0002.png"] {
        ...
        fs::write(
            ...
            serde_json::to_vec(&json!({
                "sourceGroups": [{"adapter": "Insta360DualTrackAdapter"}]
            }))
            .unwrap(),
        )
        .unwrap();
        ...
        let pairs = fs::read_to_string(temp.path().join("metadata/pairs.txt")).unwrap();
        assert!(pairs.lines().all(|pair| {
            pair.starts_with("lens0/") && pair.contains(" lens0/")
                || pair.starts_with("lens1/") && pair.contains(" lens1/")
        }));
    }

    #[test]
    fn dji_bootstrap_mapper_result_migrates_to_refinable_nominal_rig() {
        let temp = tempfile::tempdir().unwrap();
        for lens in ["lens0", "lens1"] {
            fs::create_dir_all(temp.path().join("images").join(lens)).unwrap();
            for name in ["frame0001.png", "frame0002.png"] {
        ...
        .unwrap();
        fs::write(
            temp.path().join("metadata/align.checkpoint.json"),
            serde_json::to_vec(&json!({"effectiveMapper": "bootstrap_mapper"})).unwrap(),
        )
        .unwrap();
        fs::write(
            temp.path().join("rig_config.json"),
            serde_json::to_vec(&json!([{"cameras": [
                ...
                {
                    "image_prefix": "lens1/",
                    "cam_from_rig_rotation": [0.01, 0.0, 0.99995, 0.0],
                    "cam_from_rig_translation": [0.03, -0.01, 0.01]
                }
            ]}]))
            .unwrap(),
        )
