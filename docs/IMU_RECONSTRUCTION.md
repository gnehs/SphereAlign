...
4. 首次 `auto` 若沒有 calibration marker，先保留一個 incremental seed。程式從 seed 的 camera-from-world rotations 與每來源 fused attitude 估 angular-speed time offset，再解 `A X = X B`；低樣本、低激發、單軸退化、低相關或高 residual 都會拒絕。
...
6. 若 gravity coverage 至少 80%、rig 外參完整、focal prior 有效且 CLI options 可用，`auto` 會建立 global candidate。候選模型包含 rigs/frames 且驗證成功後才提交；失敗時保留 incremental seed。
