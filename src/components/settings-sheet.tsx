...
export function SettingsSheet({
  ...
  const doctorHasChecked = doctor.checkedAt !== "Not checked yet";
  ...
  return (
    <Sheet open={settingsOpen} onOpenChange={setSettingsOpen}>
      <SheetContent className="w-[min(420px,100vw)] gap-0 border-l bg-card p-0 data-[side=right]:sm:max-w-[420px]" side="right">
        <SheetHeader className="border-b px-6 pt-6 pb-4"><SheetTitle className="text-base font-semibold"><Trans>Settings</Trans></SheetTitle><SheetDescription className="mt-2 text-sm leading-relaxed"><Trans>Manage language, appearance, and runtime diagnostics.</Trans></SheetDescription></SheetHeader>
        <div className="scroll-fade-y scroll-fade-8 flex-1 overflow-y-auto px-6">
          ...
          <section className="border-b py-4 last:border-b-0">
            <div className="mb-3 flex flex-col items-stretch gap-3">
              ...
              <div className="grid w-full grid-cols-2 gap-2 max-[360px]:grid-cols-1 [&_[data-slot=button]]:w-full [&_[data-slot=button]]:min-w-0" role="group" aria-label={t`Diagnostic actions`}>
                ...
                <Button type="button" size="sm" className={doctorLoading ? "[&_svg]:animate-spin [&_svg]:[animation-duration:750ms] motion-reduce:[&_svg]:animate-none" : undefined} disabled={doctorLoading} onClick={() => void runDoctor(colmapPath)}><RefreshCw data-icon="inline-start" />{doctorLoading ? <Trans>Checking</Trans> : doctorHasChecked ? <Trans>Check again</Trans> : <Trans>Run check</Trans>}</Button>
              </div>
            </div>
            <div className="mb-4 flex flex-col gap-2.5">
              {!doctorHasChecked ? (
                <Alert className="bg-muted text-muted-foreground" role="status">
                  <MonitorCog />
                  <AlertTitle><Trans>Runtime environment</Trans></AlertTitle>
                  <AlertDescription><Trans>Run the environment check to confirm whether CUDA, hardware decoding, or the CPU will be used.</Trans></AlertDescription>
                </Alert>
              ) : <Alert className={doctorEssentialReady ? "border-success/30 bg-success/10 text-success [&_[data-slot=alert-description]]:text-muted-foreground" : undefined} variant={doctorEssentialReady ? "default" : "destructive"} role={doctorEssentialReady ? "status" : "alert"}>
              ...
              {doctorHasChecked && performanceStatus !== "ready" && <Alert className={cn("[&_[data-slot=alert-description]]:leading-relaxed [&_[data-slot=alert-description]]:text-muted-foreground", performanceStatus === "warning" ? "border-warning/40 bg-warning/10 text-warning" : "bg-muted text-muted-foreground")} role={performanceStatus === "warning" ? "alert" : "status"}>
                <Gauge />
            ...
            {doctorHasChecked && isWindowsPlatform && <Field><FieldLabel htmlFor="colmap-path"><Trans comment="Path to the COLMAP executable used by the Windows runtime.">COLMAP executable</Trans></FieldLabel><FieldContent><div className="flex items-center gap-2 [&_input]:flex-1"><Input id="colmap-path" value={colmapPath} placeholder={t`Leave blank to detect from PATH`} onChange={(event) => setColmapPath(event.currentTarget.value)} /><Button type="button" variant="outline" size="sm" onClick={() => void openColmapPicker()}>{t`Change path`}</Button></div><FieldDescription><Trans>For the official Windows portable build, select bin/COLMAP.exe. Do not select the root COLMAP.bat launcher.</Trans></FieldDescription></FieldContent></Field>}
            {doctorHasChecked && <><div className="flex items-start gap-2.5 py-2.5"><MonitorCog className="mt-0.5 size-4 shrink-0 text-primary" strokeWidth={1.75} /><span className="flex min-w-0 flex-1 flex-col gap-1"><strong className="text-base font-medium text-foreground">{localiseUserMessage(doctor.platform)}</strong><small className="text-sm leading-snug break-anywhere text-muted-foreground">{localiseUserMessage(doctor.summary)}</small></span></div>
            <div className="border-t">{doctor.items.map((item) => { const Icon = iconForDiagnostic(item.label); return <article className="flex min-w-0 items-start gap-2.5 border-t py-2.5 first:border-t-0" key={item.label}><Icon className={cn("mt-0.75 size-4 shrink-0 text-muted-foreground", item.status === "ready" && "text-success", item.status === "warning" && "text-destructive")} strokeWidth={1.75} /><div className="flex min-w-0 flex-1 flex-col gap-1"><div className="flex min-w-0 items-start gap-2"><span className="flex min-w-0 flex-1 flex-col gap-1"><small className="text-sm leading-snug break-anywhere text-muted-foreground">{diagnosticItemLabel(item.label)}</small><strong className="text-base font-medium text-foreground">{localiseUserMessage(item.value)}</strong></span><Badge className="shrink-0 text-sm" variant={item.status === "warning" ? "destructive" : "outline"}>{diagnosticStatusLabel(item.status)}</Badge></div><p className="mt-0.5 text-sm leading-relaxed break-anywhere text-muted-foreground">{localiseUserMessage(item.detail)}</p>{item.details && item.details.length > 0 && <Accordion className="mt-1"><AccordionItem value={`${item.label}-details`}><AccordionTrigger className="w-fit py-1 text-sm font-medium text-primary"><Trans>View details</Trans></AccordionTrigger><AccordionContent><ul className="mt-2 mb-0.5 flex list-none flex-col gap-1.5 text-sm leading-relaxed text-muted-foreground">{item.details.map((detail) => <li className="border-l-2 pl-3 break-anywhere" key={detail}>{localiseUserMessage(detail)}</li>)}</ul></AccordionContent></AccordionItem></Accordion>}</div></article>; })}</div>
