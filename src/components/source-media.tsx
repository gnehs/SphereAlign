...
export function SourceListItem({
  ...
  const cameraIdentity = [source.cameraBrand, source.cameraModel].filter(Boolean).join(" · ");
  return (
    <div className="flex min-w-0 items-center gap-2.5 border-t px-2 py-2 first:border-t-0">
      ...
      <span className="flex min-w-0 flex-1 flex-col gap-0.5">
        <span className="flex min-w-0 items-center gap-1.5">
          ...
          {cameraIdentity && <Badge variant="outline" className="h-5 max-w-40 truncate px-1.5 text-xs font-normal" title={cameraIdentity}>{cameraIdentity}</Badge>}
