  ...
  const renderSettingsFields = () => (
    <div className="settings-form">
      <FieldGroup>
        ...
        <Field><FieldLabel>遮罩／物件與天空</FieldLabel><FieldContent><div className="mask-chip-list">{MASK_CLASSES.map((maskClass) => { const selected = settingsDraft.mask.classes.includes(maskClass); return <Button key={maskClass} type="button" variant={selected ? "secondary" : "outline"} size="sm" aria-pressed={selected} onClick={() => setSettingsDraft((current) => ({ ...current, mask: { ...current.mask, classes: selected ? current.mask.classes.filter((value) => value !== maskClass) : [...current.mask.classes, maskClass] } }))}>{MASK_CLASS_LABELS[maskClass]}</Button>; })}</div><div className="control-line"><Checkbox checked={settingsDraft.mask.maskSky} onCheckedChange={(checked) => setSettingsDraft((current) => ({ ...current, mask: { ...current.mask, maskSky: checked === true } }))} /><span>遮蔽天空</span><span className="range-label">信心度 {settingsDraft.mask.confidence}%</span></div><input className="range-input" type="range" min={10} max={98} aria-label="遮罩信心度" value={settingsDraft.mask.confidence} onChange={(event) => { const value = Number(event.currentTarget.value); setSettingsDraft((current) => ({ ...current, mask: { ...current.mask, confidence: value } })); }} /><div className="input-with-button model-dir-input"><Input value={settingsDraft.mask.modelDir} placeholder="模型資料夾（未指定時自動探索）" onChange={(event) => { const value = event.currentTarget.value; setSettingsDraft((current) => ({ ...current, mask: { ...current.mask, modelDir: value } })); }} /><Button type="button" variant="outline" size="sm" onClick={() => void openModelPicker()}>選擇</Button></div></FieldContent></Field>
